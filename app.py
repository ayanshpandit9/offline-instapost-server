...
@app.route('/')
def index():
    return render_template_string(LOGIN_PAGE, date=datetime.now().strftime('%d-%m-%Y'))
...
def login():
    ...
    try:
        ...
        return render_template_string(LOGIN_PAGE, error=f"Login failed: {str(e)}", date=datetime.now().strftime('%d-%m-%Y'))
...
def comment():
    ...
    try:
        ...
        return render_template_string(MAIN_PAGE, message=f"Session expired: {str(e)}", message_type="error", date=datetime.now().strftime('%d-%m-%Y'))
